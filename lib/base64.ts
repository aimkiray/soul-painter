export function normalizeToDataUrl(raw: string) {
  ...
  s = s.replace(/\s+/g, '');
  if (!/^[A-Za-z0-9+/]*={0,2}$/.test(s)) throw new Error('包含非法的 Base64 字符');
  let bin: string;
  try {
  ...
  // Only formats the asset store accepts (MIME_TO_EXT in lib/chat-assets.ts)
  // may be sniffed — anything else would display once then vanish on reload.
  if (!mime) throw new Error('无法识别的图片格式');
